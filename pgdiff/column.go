...
func (c *ColumnSchema) Add() {
	...
	if strings.ToUpper(c.get("data_type")) == "CHARACTER VARYING" {
		...
		if strings.ToUpper(c.get("data_type")) == "CHARACTER" {
			fmt.Printf("ALTER TABLE \"%s\".\"%s\" ADD COLUMN \"%s\" %s(%s)", schema, c.get("table_name"), c.get("column_name"), strings.ToUpper(c.get("data_type")), c.get("character_maximum_length"))
		} else {
			fmt.Printf("ALTER TABLE \"%s\".\"%s\" ADD COLUMN \"%s\" %s", schema, c.get("table_name"), c.get("column_name"), strings.ToUpper(c.get("data_type")))
		}
...
func (c *ColumnSchema) Drop() {
	...
	DO
		...
		$$;
	`,
		c.get("table_schema"), c.get("table_name"), c.get("table_schema"), c.get("table_name"), c.get("column_name"))
...
func (c *ColumnSchema) Change(obj interface{}) {
	...
	if c.get("data_type") == c2.get("data_type") {
		if c.get("data_type") == "character varying" || c.get("data_type") == "character" {
			...
			if !max1Valid && !max2Valid {
				...
				fmt.Printf("ALTER TABLE \"%s\".\"%s\" ALTER COLUMN \"%s\" TYPE %s(%s);\n", c2.get("table_schema"), c.get("table_name"), c.get("column_name"), currentType, max1)
			}
		}
	}
	...
	if c.get("data_type") != c2.get("data_type") {
		...
		if strings.HasPrefix(c.get("data_type"), "character") {
			...
			fmt.Printf("ALTER TABLE \"%s\".\"%s\" ALTER COLUMN \"%s\" TYPE %s(%s);\n", c2.get("table_schema"), c.get("table_name"), c.get("column_name"), strings.ToUpper(c.get("data_type")), max1)
		} else {
			fmt.Printf("ALTER TABLE \"%s\".\"%s\" ALTER COLUMN \"%s\" TYPE %s;\n", c2.get("table_schema"), c.get("table_name"), c.get("column_name"), strings.ToUpper(c.get("data_type")))
		}
	}
