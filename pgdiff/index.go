...
func (c *IndexSchema) Add() {
	...
	// Check if exists another primary key with same name and drop
	fmt.Printf(`
	DO
$$
    DECLARE
        internal_table_schema    CHARACTER(255) = NULL;
        internal_table_name      CHARACTER(255) = NULL;
        internal_constraint_name CHARACTER(255) = NULL;
    BEGIN
        SELECT kcu.table_schema,
               tco.table_name,
               tco.constraint_name
        INTO internal_table_schema, internal_table_name, internal_constraint_name
        FROM information_schema.table_constraints tco
                 JOIN information_schema.key_column_usage kcu
                      ON kcu.constraint_name = tco.constraint_name
                          AND kcu.constraint_schema = tco.constraint_schema
        WHERE tco.constraint_type = 'PRIMARY KEY'
          AND tco.constraint_name = '%s'
        LIMIT 1;
        IF (internal_constraint_name IS NOT NULL) THEN
            EXECUTE 'ALTER TABLE "' || internal_table_schema || '"."' || internal_table_name || '" DROP CONSTRAINT "' ||
...
$$;
	...
	// Check if exists another index with same name and drop
	fmt.Printf(`
	DO
$$
    DECLARE
    ...
    BEGIN
        ...
        IF internal_constraint_name IS NOT NULL THEN
            EXECUTE 'DROP INDEX "' || internal_table_schema || '"."' || internal_constraint_name || '";';
        END IF;
    END;
$$;
	...
	if c.get("constraint_def") != "null" {
		...
		if c.get("pk") == "true" {
			// Delete Old prymary key if exists
			fmt.Printf(`
			DO
			$$
				DECLARE
...
func (c *IndexSchema) Drop() {
	if c.get("constraint_def") != "null" {
		...
		// 			WHERE table_schema = '%s' AND table_name = '%s') THEN
		// 		ALTER TABLE "%s"."%s" DROP CONSTRAINT "%s" CASCADE; -- %s
		// 		END IF;
		// 	END;
		// 	$$;
		// `,
		// 			c.get("schema_name"), c.get("table_name"), c.get("schema_name"), c.get("table_name"), c.get("index_name"), c.get("constraint_def"))
	}
...
DO
	...
	BEGIN
		IF EXISTS(SELECT
			...
			INNER JOIN pg_catalog.pg_namespace AS n ON (c2.relnamespace = n.oid)
			WHERE n.nspname = '%s'
			AND c2.relname = '%s') THEN
				DROP INDEX "%s"."%s";
		END IF;
	END;
	$$;
`,
		c.get("schema_name"), c.get("index_name"), c.get("schema_name"), c.get("index_name"))
}
	...
	if c.get("constraint_def") != c2.get("constraint_def") {
		...
		if c.get("constraint_def") == "null" {
			...
			if c.get("index_def") == c2.get("index_def") {
				...
				fmt.Printf("DROP INDEX \"%s\".\"%s\"; -- %s \n", c.get("schema_name"), c2.get("index_name"), c2.get("index_def"))
			}
			// WIP
